 ...
 * a live USD/THB reference rate on the way out (see `usdRate`) — a
...
export class BitkubProvider implements MarketDataProvider {
  ...
  /**
   * THB per 1 USD, from the shared keyless FX chain in
   * `@zframes/data-primitives/fx-rate` — cached, plausibility-checked, and
   * stale-on-error there. The rate gates *every* number this provider emits, so
   * it is deliberately not a single upstream.
   */
